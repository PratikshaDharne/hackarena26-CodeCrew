      ...
      <div className="max-w-6xl mx-auto px-6 py-8 pb-24 md:pb-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Your spending overview</p>
          </div>
          <Link
            to="/scan"
        ...
        {receipts && receipts.length > 0 ? (
          <>
            <div className="grid lg:grid-cols-5 gap-6 mb-8">
              {monthlyData.length > 0 && (
                <div className="glass-card p-6 lg:col-span-3">
                  <h3 className="font-display font-semibold mb-4">Spending by Month</h3>
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={monthlyData}>
                      <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: "hsl(215 14% 50%)", fontSize: 12 }} />
                      <YAxis axisLine={false} tickLine={false} tick={{ fill: "hsl(215 14% 50%)", fontSize: 12 }} />
              ...
              {categoryTotals.length > 0 && (
                <div className="glass-card p-6 lg:col-span-2">
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                      <Pie data={categoryTotals} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} strokeWidth={0}>
                        {categoryTotals.map((entry) => (
                          <Cell key={entry.name} fill={CATEGORY_COLORS[entry.name] || "hsl(215, 14%, 50%)"} />
                        ))}
                      </Pie>
                      <Tooltip contentStyle={{ background: "hsl(220 18% 8%)", border: "1px solid hsl(220 14% 16%)", borderRadius: 8 }} />
                    </PieChart>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {categoryTotals.map((c) => (
                      <span key={c.name} className="text-xs text-muted-foreground flex items-center gap-1">
                        <span className="w-2 h-2 rounded-full" style={{ background: CATEGORY_COLORS[c.name] || "#888" }} />
                        {c.name}
                      </span>
                    ))}
                  </div>
            ...
            <div className="glass-card p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link to="/receipts" className="text-sm text-primary hover:underline">View all</Link>
              </div>
              <div className="space-y-3">
                {receipts.slice(0, 5).map((r) => (
                  <div key={r.id} className="flex items-center justify-between py-3 border-b border-border/30 last:border-0">
                    <div className="flex items-center gap-3">
                      <span className="text-xl">{CATEGORY_ICONS[r.category || "Food"] || "📦"}</span>
                      <div>
                        <p className="font-medium text-sm">{r.store_name || "Unknown Store"}</p>
                        <p className="text-xs text-muted-foreground">{r.purchase_date} · {r.category}</p>
                      </div>
                    </div>
                    <span className="font-display font-semibold">${(r.total_amount || 0).toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="glass-card p-12 text-center">
            <p className="text-muted-foreground mb-4">No receipts yet. Scan your first receipt!</p>
            <Link to="/scan" className="inline-flex items-center gap-2 px-5 py-3 bg-primary text-primary-foreground font-semibold rounded-xl">
              <ScanLine className="w-4 h-4" /> Scan Receipt
            </Link>
          </div>
        )}
